...
::: warning Mock 데이터의 주문 날짜는 고정 — 반품 테스트는 '배송 중' 주문을 쓰세요
`cs_lookup_order`의 주문 데이터는 2026년 6월 날짜로 고정되어 있습니다. 반품 정책은 "배송완료일로부터 N일 이내"로 계산되므로, 워크샵을 진행하는 **오늘 날짜가 이미 배송완료일로부터 14일(전자기기)~30일을 넘겼다면, 에스컬레이션이 아니라 "반품 기한 초과"로 거부됩니다.**
아래 테스트는 이미 배송 완료된 주문이 아니라 **아직 배송 중(IN_TRANSIT)인 주문**을 사용해 이 문제를 피합니다 — 반품 접수 시점에 배송완료일 자체가 없어서 기한 계산이 발생하지 않습니다.
:::
...
{"prompt": "주문 ORD-2024-555 배송 어디쯤 왔나요?", "actor_id": "C002", "session_id": "policy-test-002-bbbbbbbbbbbbbbbbbbbb"}
JSON
...
### 테스트 3: 고가 반품 (65,000원 — 에스컬레이션 발동!)

```bash
cat > /tmp/policy-3.json <<'JSON'
{"prompt": "주문 ORD-20260625-010 환불 요청합니다. 단순변심이에요.", "actor_id": "C002", "session_id": "policy-test-003-cccccccccccccccccccc"}
JSON
agentcore invoke --prompt-file /tmp/policy-3.json
```

::: details ✅ 예상 결과 — 에스컬레이션
```
🤖 Agent: ORD-20260625-010 주문을 확인했습니다.

- 상품: 히알루론산 세럼(35,000원) + 클렌징 폼(15,000원×2)
- 결제금액: 65,000원
- 배송상태: 배송 중 (아직 배송완료 전이라 반품 기한 계산은 미개봉/개봉 조건만 적용)
- 반품 사유: 단순변심

⚠️ 환불 금액이 50,000원을 초과하여, 
CS 팀 리더에게 전달하겠습니다.

- 에스컬레이션 사유: 환불 금액 기준 초과 (65,000원 > 50,000원)
- 예상 처리 시간: 24시간 이내
- 담당: CS 팀 리더
