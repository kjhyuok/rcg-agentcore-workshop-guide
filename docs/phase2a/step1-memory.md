...
- **Phase 2** — Agent는 여전히 요청마다 새로 뜨지만, **AgentCore Memory로 이전 맥락을 끌어와 프롬프트에 주입** → "기억하는 것처럼" 동작

> **핵심:** 이 워크샵은 전통적 멀티턴(대화 메시지 배열을 매번 다시 전달)이 아니라, **멀티턴 역할까지 Memory가 대신**합니다. 그래서 Phase 1엔 "멀티턴 코드"가 따로 없고, Phase 2에서 Memory를 붙이는 것으로 맥락 유지를 구현합니다.
