...
::: details 🧩 헷갈리기 쉬운 용어 정리 — 세션 · 멀티턴 · 메모리 (· 프롬프트 캐싱)
비슷해 보이지만 **계층과 목적이 다릅니다.**

| 용어 | 한 문장 | 시간 범위 |
|------|---------|-----------|
| **세션(session)** | 지금 이 대화 창 하나 (`session_id`로 식별) | 이번 접속 |
| **멀티턴** | 한 세션 **안에서** 문맥을 이어가는 여러 번의 주고받음 | 세션 내부 |
| **메모리(Memory)** | 세션을 **넘어** 그 고객(`actor_id`)을 기억 | 영구 |
| **프롬프트 캐싱** | LLM 호출 **비용·속도** 최적화 (기억과 무관) | — |

**이 워크샵의 중요한 설계 포인트:**

AgentCore Runtime은 **요청마다 Agent를 새로 만들어** 실행합니다(`agent = Agent(...)` → `stream_async(prompt)`). 즉 **Agent 객체가 요청 사이에 살아있지 않아, 대화가 저절로 이어지지 않습니다.**

- **Phase 1** — 상태 없음. 매 요청이 완전한 백지 (멀티턴 ❌, 메모리 ❌)
- **Phase 2A** — Agent는 여전히 요청마다 새로 뜨지만, **AgentCore Memory로 이전 맥락을 끌어와 프롬프트에 주입** → "기억하는 것처럼" 동작

> **핵심:** 이 워크샵은 전통적 멀티턴(대화 메시지 배열을 매번 다시 전달)이 아니라, **멀티턴 역할까지 Memory가 대신**합니다. 그래서 Phase 1엔 "멀티턴 코드"가 따로 없고, Phase 2A에서 Memory를 붙이는 것으로 맥락 유지를 구현합니다.

**프롬프트 캐싱**은 기억이 아니라 순수 성능 최적화 기술이며, **이 워크샵 코드에서는 사용하지 않습니다** — 여기서는 신경 쓰지 않아도 됩니다.
:::
