...
!!! tip "Step 1에서 쓰던 터미널을 그대로 이어서 사용하세요"
    Step 1에서 연 터미널에 `~/workshop/starter-code`, venv, 환경변수가 이미 살아있다면 **"Open Folder"는 건너뛰어도 됩니다.**
    아래 "Open Folder" 절차는 세션이 끊겼거나 터미널/환경변수가 초기화됐을 때만 수행하세요.
