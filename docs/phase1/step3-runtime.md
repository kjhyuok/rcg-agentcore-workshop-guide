...
Agent가 토큰을 실시간으로 스트리밍하기 때문에, 한 번에 완성된 응답이 아니라 텍스트 조각(event)이 생성되는 즉시 여러 번 흘러나옵니다.
답을 다 만들 때까지 기다리지 않고 생성되는 즉시 보여주기 때문에 체감 응답 속도가 훨씬 빠릅니다. 화면에 순서대로 이어지는 텍스트가 최종 응답입니다.
:::
